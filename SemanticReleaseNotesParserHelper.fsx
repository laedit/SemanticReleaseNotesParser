...
module SemanticReleaseNotesParser =
    ...
    let private findExe toolPath =
        if toolPath |> isNullOrEmpty then
            let found = [
                            Seq.singleton (findToolInSubPath "SemanticReleaseNotesParser.exe" ("tools" @@ "semanticreleasenotes-parser"))
                            Seq.singleton ("tools" @@ "SemanticReleaseNotesParser")
                            Seq.singleton (environVar "ProgramData" @@ "chocolatey" @@ "lib" @@ "semanticreleasenotes-parser")
                            pathDirectories
                        ]
                        |> Seq.concat
                        |> Seq.map (fun directory -> if hasExt ".exe" directory then directory else directory @@ "SemanticReleaseNotesParser.exe")
